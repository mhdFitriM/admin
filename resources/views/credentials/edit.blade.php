                    ...
                    <form method="POST" action="{{ route('credentials.update', $credential) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="service" id="service"
                                value="{{ old('service', $credential->service) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('service')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="api_token"
                                    {{ old('type', $credential->type) == 'api_token' ? 'selected' : '' }}>API Token
                                </option>
                                <option value="password"
                                    {{ old('type', $credential->type) == 'password' ? 'selected' : '' }}>Password
                                </option>
                                <option value="ssh_key"
                                    {{ old('type', $credential->type) == 'ssh_key' ? 'selected' : '' }}>SSH Key</option>
                                <option value="api_key"
                                    {{ old('type', $credential->type) == 'api_key' ? 'selected' : '' }}>API Key</option>
                                <option value="secret"
                                    {{ old('type', $credential->type) == 'secret' ? 'selected' : '' }}>Secret</option>
                                <option value="other"
                                    {{ old('type', $credential->type) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="value" id="value" rows="4" required
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('value', $credential->value) }}</textarea>
                            @error('value')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">This value is encrypted in the
                                database</p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $credential->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>

<x-app-layout>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="development"
                                        {{ old('status', $project->status) == 'development' ? 'selected' : '' }}>
                                        Development</option>
                                    <option value="archived"
                                        {{ old('status', $project->status) == 'archived' ? 'selected' : '' }}>Archived
                                    </option>
                                </select>
                            </div>
                                ...
                                <select name="server_id" id="server_id"
                                    ...
                                    @foreach ($servers as $server)
                                        <option value="{{ $server->id }}"
                                            {{ old('server_id', $project->server_id) == $server->id ? 'selected' : '' }}>
                                            {{ $server->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('description', $project->description) }}</textarea>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <label for="tech_stack_input"
                                    ...
                                    Stack</label>
                                <input type="text" id="tech_stack_input" placeholder="Press Enter to add"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('notes', $project->notes) }}</textarea>
                            </div>
                        </div>
    ...
    @push('scripts')
        <script>
            let techStack = @json($project->tech_stack ?? []);
            ...
            function updateTags() {
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                    ${tech}
                    <button type="button" onclick="removeTag(${index})" class="ml-2 text-green-600 hover:text-green-800 dark:text-green-400">×</button>
                </span>
            `).join('');
